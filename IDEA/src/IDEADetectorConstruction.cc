...
G4VPhysicalVolume* IDEADetectorConstruction::DefineVolumes()
{
  ...
  // World
  // -------------------------------------------------------------------------
  G4double worldR  = 2600.0 * CLHEP::mm;
  ...
  // Magnet
  // -------------------------------------------------------------------------
  G4double innerRadius_magnet = 2140. * CLHEP::mm;
  G4double outerRadius_magnet = 2440. * CLHEP::mm;
  G4double hz_magnet = 2480. * CLHEP::mm;
  G4double startAngle = 0.*deg;
  G4double spanningAngle = 360.*deg;
  G4ThreeVector positionMagnet = G4ThreeVector (0., 0., 0.);
  
  auto solidMagnet = new G4Tubs ("solidMagnet", innerRadius_magnet, outerRadius_magnet, hz_magnet, startAngle, spanningAngle);

  auto logicMagnet = new G4LogicalVolume (solidMagnet, absorberMaterial, "logicMagnet", 0, 0, 0);

  auto physMagnet =new G4PVPlacement(0,            
					positionMagnet,  
					logicMagnet,    
					"physMagnet",  
					fLogicWorld,     
  ...
  // Cylinder
  // -------------------------------------------------------------------------
  G4double innerRadius = 2440. * CLHEP::mm;
  G4double outerRadius = 2500. * CLHEP::mm;
  G4double hz = 2480. * CLHEP::mm;
  //G4double startAngle = 0.*deg;
  //G4double spanningAngle = 360.*deg;
  ...
  // CHAMBER DIMENSIONS
  G4double rwell_x_width = 413 * CLHEP::mm;
  G4double rwell_y_width = rwell_ch_thick;
  G4double rwell_z_width = 413 * CLHEP::mm;
  G4double px, py, pz;
  ...
  G4double rwell_rotAngle_L1 = (0.1653469818)*rad;
  //G4double rwell_rotAngle_B2 = (0.1674018746 + 0.08931594698)*rad;
  G4double radius_L1_1 = 2445 * CLHEP::mm;
  G4double radius_L1_2 = 2455 * CLHEP::mm;
  G4double radius_L2_1 = 2470 * CLHEP::mm;
  G4double radius_L2_2 = 2480 * CLHEP::mm;
  G4int rwell_nCopyXY_L1 = 38;
  G4int rwell_nCopyXY_L2 = 38;
  G4int rwell_nCopyZ = 12;
  // We are considering 456 chambers per layer; 2 layers in the barrel region
  fNofLayers = rwell_nCopyZ*rwell_nCopyXY_L1*rwell_nCopyXY_L2;
  
  G4RotationMatrix *zRotL1[38];  
  G4RotationMatrix *zRotL2[38];
  
  for (G4int i=0; i < rwell_nCopyXY_L1; i++)
    {
      zRotL1[i] = new G4RotationMatrix;
      zRotL1[i] -> rotateZ (i * rwell_rotAngle_L1);
      if ( i%2 == 0 )
	{
	  px = (radius_L1_1 + rwell_y_width * 0.5) * cos((twopi/4.) - i * rwell_rotAngle_L1);   
	  py = (radius_L1_1 + rwell_y_width * 0.5) * sin((twopi/4.) - i * rwell_rotAngle_L1);   
	}
      else
	{
	  px = (radius_L1_2 + rwell_y_width * 0.5) * cos((twopi/4.) - i * rwell_rotAngle_L1);   
	  py = (radius_L1_2 + rwell_y_width * 0.5) * sin((twopi/4.) - i * rwell_rotAngle_L1);   
	}
      
      for (G4int j=0; j < rwell_nCopyZ; j++)
	{
	  pz = -hz + (2*j+1) * rwell_z_width * 0.5;
	  new G4PVPlacement (zRotL1[i], G4ThreeVector (px, py, pz), rwell_ch_logic, "rwell_ch_phys", logicPreshower, false, i*rwell_nCopyZ + j);
	}
    }
  
  for (G4int i=rwell_nCopyXY_L1*rwell_nCopyZ+1; i < rwell_nCopyXY_L1*rwell_nCopyZ+1 + rwell_nCopyXY_L2; i++)
    {
      zRotL2[i] = new G4RotationMatrix;
      zRotL2[i] -> rotateZ (i * rwell_rotAngle_L1);
      if ( i%2 == 0 )
	{
	  px = (radius_L2_1 + rwell_y_width * 0.5) * cos((twopi/4.) - i * rwell_rotAngle_L1);   
	  py = (radius_L2_1 + rwell_y_width * 0.5) * sin((twopi/4.) - i * rwell_rotAngle_L1);   
	}
      else
	{
	  px = (radius_L2_2 + rwell_y_width * 0.5) * cos((twopi/4.) - i * rwell_rotAngle_L1);   
	  py = (radius_L2_2 + rwell_y_width * 0.5) * sin((twopi/4.) - i * rwell_rotAngle_L1);   
	}

      for (G4int j=0; j < rwell_nCopyZ; j++)
	{
	  pz = -hz + (2*j+1) * rwell_z_width * 0.5;
	  new G4PVPlacement (zRotL2[i], G4ThreeVector (px, py, pz), rwell_ch_logic, "rwell_ch_phys", logicPreshower, false, i*rwell_nCopyZ + j);
	}
    }
  ...
  logicMagnet->SetVisAttributes(white);
...
void IDEADetectorConstruction::ConstructSDandField()
{
  ...
  // Sensitive detectors
  //
  auto absoSD = new IDEAPreShowerSD ("AbsorberSD", "AbsorberHitsCollection", fNofLayers);
  G4SDManager::GetSDMpointer()->AddNewDetector(absoSD);
  SetSensitiveDetector ("rwell_pcb_cop_logic", absoSD);

  //EXAMPLE
  auto gapSD = new IDEAPreShowerSD ("GapSD", "GapHitsCollection", 1);
